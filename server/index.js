...
mongoose
    .connect('mongodb+srv://moses:4sHBmhoEMq4YKePr@cluster0.fdpmdnc.mongodb.net/?retryWrites=true&w=majority', connectionParams)
    .then(() => {
...
app.post("/hackathonApplication",(req,res) =>{
    hackathonApplication.create(req.body)
    .then(result => res.json(result))
    .catch(err => res.json(err))
})
...
app.put("/updateParticipants/:id",(req,res) =>{
    ...
    Hackathon.findByIdAndUpdate({_id: id}, { 
...
//     const id = req.params.id;
//     Hackathon.findById(id) // Remove curly braces around id
//         .then(hackathon => res.json(hackathon))
//         .catch(err => res.json(err));
// });
